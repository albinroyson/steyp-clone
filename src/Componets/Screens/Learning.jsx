...
export default function Learning() {
	...
	const access = 'r7aHyPh69GLonqSdk3pFfkiDrtVJ0I'
        ...
        useEffect(() => {
            axios
            .get('https://learn.steyp.com/api/v1/learn/designations/tech-schooling/',
            {
                headers:{
         		...
         		<Listitem onClick={()=>{
					...
					// }array filter in react mala
					 ...
					 selectedId.includes(item.id) ? 
						setSelectedId(selectedId.filter(id=> item.id !== id))
						: setSelectedId([...selectedId,item.id]) 
					
				 }
				}
				 className={item.status === "completed" ? "change" : ""}>	
					<Leftcontianer>
						<Subjectimage src={item.image} />
					</Leftcontianer>
					<Rightcontainer>
					<Position>#<Num>1</Num></Position>
						<Name>{item.name}</Name>
					<Bottomdiv>
						<Skilldiv>
							<Skillicon src={require("../../assests/Img/books.png")} />
							<Skillcount>9 skills</Skillcount>
					 	 </Skilldiv>
					  	<Timediv>
							<Timeicon src={require("../../assests/Img/wall-clock.png")} alt="clock" />
